...
async function fillPersonalInfo(
	...
	await page
		...
		.fill(overrides.name ?? "John Doe");
	await page
		.locator('input[placeholder*="000000000V"]')
		.fill(overrides.nic ?? "200301503249");
	await page
		.locator("select")
		.first()
		.selectOption(overrides.gender ?? "male");
	await page
		.locator('input[placeholder*="email address"]')
		.fill(overrides.email ?? "john@sci.sjp.ac.lk");
...
async function fillAcademicFieldsInternal(
	page: Page,
	overrides: Record<string, string> = {},
) {
	await page
		.locator("select")
		.nth(0)
		.selectOption(overrides.faculty ?? "FOT");
	await page
		.locator('input[placeholder="123456"]')
		.nth(0)
		.fill(overrides.regNo ?? "123456");
	await page
		.locator('input[placeholder="username"]')
		.fill(overrides.uniEmailLocal ?? "ict23922");
	await page
		.locator("select")
		.nth(1)
		.selectOption(overrides.academicYear ?? "year-3");
	await page
		.locator("select")
		.nth(2)
		.selectOption(overrides.degree ?? "Bachelor of ICT (Hons)");
}

async function fillAcademicInfoInternal(
	page: Page,
	overrides: Record<string, string> = {},
) {
...
async function _selectAwards(page: Page, awards: string[]) {
...
async function _fillBestInnovator(page: Page) {
...
async function fillBestCSR(page: Page) {
	...
	await page
		.locator('input[placeholder*="advisor email"]')
		.fill("advisor@club.com");
	...
	await page
		.locator('input[placeholder*="president name"]')
		.fill("Bob President");
	await page
		.locator('input[placeholder*="7X XXX XXXX"]')
		.nth(1)
		.fill("772222222");
	await page
		.locator('input[placeholder*="president email"]')
		.fill("president@club.com");
}
...
test.describe("Step 0 – Applicant Type", () => {
	test("renders both options and next is disabled until selection", async ({
		page,
	}) => {
		await goToForm(page);
...
test.describe("Step 2 – Academic Info", () => {
	test("shows graduation year dropdown for recent graduates", async ({
		page,
	}) => {
		await goToForm(page);
		await selectInternal(page);
	...
	test("does not offer graduation years outside 2023–2026", async ({
		page,
	}) => {
		await goToForm(page);
		await selectInternal(page);
...
test.describe("Step 3 – Award Selection Rules", () => {
	test("internal USJ can select up to 2 main awards plus BESA Inter University", async ({
		page,
	}) => {
		await goToForm(page);
		await selectInternal(page);
		...
		for (const award of awards) {
			await page
				.getByRole("checkbox", { name: new RegExp(award, "i") })
				.check();
		}
		await expect(
			page.getByRole("checkbox", { name: /Best Innovator/i }),
		).toBeDisabled();
		await page
			.getByRole("checkbox", { name: /BESA.*Inter University/i })
			.check();
		await page.getByRole("button", { name: /Continue/i }).click();
	});

	test("external can only select Best Innovator and BESA Inter University", async ({
		page,
	}) => {
		await goToForm(page);
		...
		await page
			.locator('input[placeholder*="faculty name"]')
			.fill("Faculty of Science");
		await page.locator("select").first().selectOption("University of Colombo");
		await page
			.locator('input[placeholder*="registration number"]')
			.fill("EXT12345");
		await page
			.locator('input[placeholder*="university email"]')
			.fill("jane@cmb.ac.lk");
		await page.locator("select").nth(1).selectOption("year-3");
		...
		await expect(
			page.getByRole("checkbox", { name: /Best Leader/i }),
		).toHaveCount(0);
		await expect(
			page.getByRole("checkbox", { name: /Best Innovator/i }),
		).toBeEnabled();
		await expect(
			page.getByRole("checkbox", { name: /BESA.*Inter University/i }),
		).toBeEnabled();
	...
	test("recent-graduate forced to Best Innovator only", async ({ page }) => {
		...
		const checkboxes = page.getByRole("checkbox");
		const _count = await checkboxes.count();
...
test.describe("Step 4 – Award Questions", () => {
	test("Best Innovator requires completion confirmation", async ({ page }) => {
		...
		await page.getByRole("button", { name: /Continue/i }).click();
		await expect(
			page.getByText(/innovation is more than 75% completed/i),
		).toBeVisible();
	});

	test("CSR validates advisor and president emails are different", async ({
		page,
	}) => {
		await goToForm(page);
		await selectInternal(page);
	...
	test("CSR accepts different advisor and president emails", async ({
		page,
	}) => {
		await goToForm(page);
		await selectInternal(page);
...
test.describe("Step 5 – Declaration & Swipe", () => {
	test("swipe button is disabled until all declarations checked", async ({
		page,
	}) => {
		await goToForm(page);
		await selectInternal(page);
	...
	test("swipe button enables after all declarations checked", async ({
		page,
	}) => {
		await goToForm(page);
		await selectInternal(page);
...
test.describe("Full Happy Path", () => {
	test("internal USJ student with Best Innovator and Best CSR submits successfully", async ({
		page,
	}) => {
		// ════════════════════════════════════
		...
		// ════════════════════════════════════
		await expect(
			page.getByRole("heading", { name: "Academic Information" }),
		).toBeVisible();
		...
		// ════════════════════════════════════
		await expect(
			page.getByRole("heading", { name: "Award Selection" }),
		).toBeVisible();
		await page.getByRole("checkbox", { name: /Best Innovator/i }).check();
		await page.getByRole("checkbox", { name: /Best CSR/i }).check();
		await page.getByRole("button", { name: /Continue/i }).click();
		...
		// Best CSR section
		await page
			.locator('input[placeholder*="advisor name"]')
			.fill("Dr. Alice Advisor");
		await page
			.locator('input[placeholder*="advisor email"]')
			.fill("advisor@club.lk");
		await page
			.locator('input[placeholder*="member name"]')
			.fill("Charlie Member");
		const csrPhones = page.locator('input[placeholder*="7X XXX XXXX"]');
		await csrPhones.nth(0).fill("771111111");
		await page
			.locator('input[placeholder*="president name"]')
			.fill("Diana President");
		await page
			.locator('input[placeholder*="7X XXX XXXX"]')
			.nth(1)
			.fill("772222222");
...
test.describe("Edge Cases", () => {
	...
	test("swipe button is disabled when declarations are not all checked", async ({
		page,
	}) => {
		await goToForm(page);
		await selectInternal(page);

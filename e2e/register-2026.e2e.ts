...
const generateUniqueNIC = () =>
	`${Math.floor(100000000 + Math.random() * 900000000)}V`;
const generateUniquePhone = () =>
	`77${Math.floor(1000000 + Math.random() * 9000000)}`;
const generateUniqueEmail = (prefix: string) =>
	`${prefix}${Date.now()}@example.com`;
const generateUniqueUSJEmail = () =>
	`${Math.floor(100000 + Math.random() * 900000)}@fot.sjp.ac.lk`;
...
test.describe("JESA 2026 Registration Form", () => {
	...
	test("Scenario 1: Complete Internal USJ Student Submission", async ({
		page,
	}) => {
		// Step 0: Landing
		...
		await expect(
			page.locator("h2").filter({ hasText: "Personal Information" }),
		).toBeVisible();
		await page
			.getByPlaceholder("Enter your display name")
			.fill("Test Internal User");
		await page
			.getByPlaceholder("000000000V or 000000000000")
			.fill(generateUniqueNIC());
		await page.getByRole("combobox").first().selectOption("male");
		await page
			.getByPlaceholder("Enter your email address")
			.fill(generateUniqueEmail("internal"));
		await page
			.getByPlaceholder("7X XXX XXXX")
			.first()
			.fill(generateUniquePhone());
		await page
			.getByPlaceholder("7X XXX XXXX")
			.nth(1)
			.fill(generateUniquePhone());
		...
		try {
			await expect(
				page.locator("h2").filter({ hasText: "Academic Information" }),
		...
		await page
			...
			.fill(`${Math.floor(100000 + Math.random() * 900000)}`); // FOT email prefix
		await page.getByRole("combobox").nth(1).selectOption("year-3"); // Academic Year
		await page
			.getByRole("combobox")
			.nth(2)
			.selectOption("Bachelor of ICT (Hons)"); // Degree
		await page.getByRole("button", { name: /Continue/i }).click();

		// Step 3: Award Selection
		await expect(
			page.locator("h2").filter({ hasText: "Award Selection" }),
		).toBeVisible();
		...
		await expect(
			page.locator("h2").filter({ hasText: "Award Questions" }),
		).toBeVisible();
		await page.getByRole("combobox").first().selectOption("Technology");
		await page
			.locator("label")
			.filter({
				hasText: "I declare that my innovation is more than 75% completed",
			})
			.click();
		await page.getByRole("button", { name: /Continue/i }).click();
		...
		// Step 5: Declaration
		await expect(
			page.locator("h2").filter({ hasText: "Declaration" }),
		).toBeVisible();
		await page
			.locator("label")
			.filter({ hasText: "I confirm all provided information is true." })
			.click();
		await page
			.locator("label")
			.filter({
		...
		await page
			.locator("label")
			.filter({ hasText: "I agree that not attending physical interview" })
			.click();
		await page
			.locator("label")
			.filter({ hasText: "I permit the organizing committee to verify" })
			.click();
		await page
			.locator("label")
			.filter({ hasText: "I consent to the use of my name" })
			.click();
		...
		await expect(
			page.locator("text=Registration Successful").first(),
		).toBeVisible();
	});
	...
	test("Scenario 2: Complete External Student Submission", async ({ page }) => {
		...
		await page
			.getByPlaceholder("Enter your email address")
			.fill(generateUniqueEmail("external"));
		await page
			.getByPlaceholder("7X XXX XXXX")
			.first()
			.fill(generateUniquePhone());
		...
		await page
			.getByRole("combobox")
			.first()
			.selectOption("University of Colombo");
		await page
			.getByPlaceholder("Enter your registration number")
			.fill(`2021/SC/${Math.floor(100 + Math.random() * 900)}`);
		await page
			.getByPlaceholder("Enter your university email")
			.fill(`sci${Date.now()}@cmb.ac.lk`);
		await page.getByRole("combobox").nth(1).selectOption("year-4");
		await page
			.getByPlaceholder("Enter your degree name")
	...
	test("Scenario 3: Validation Error on Duplicate Submission", async ({
		page,
	}) => {
		...
		await page
			.getByRole("combobox")
			.nth(2)
			.selectOption("Bachelor of ICT (Hons)");
		await page.getByRole("button", { name: /Continue/i }).click();
		...
		await expect(
			page.locator(
				"text=An application with the same NIC and WhatsApp Number and University Email already exists",
			),
		).toBeVisible();
	});

	test("Scenario 4: Award Limits (Internal max 3, 2 main + BESA)", async ({
		page,
	}) => {
		await page.getByText("internal student", { exact: false }).click();
		await page.getByRole("button", { name: /Continue/i }).click();
		...
		await page
			.getByPlaceholder("Enter your email address")
			.fill(generateUniqueEmail("limit"));
		await page
			.getByPlaceholder("7X XXX XXXX")
			.first()
			.fill(generateUniquePhone());
		...
		await page
			.getByPlaceholder("username")
			.fill(`${Math.floor(100000 + Math.random() * 900000)}`);
		...
		await page
			.locator("label")
			.filter({ hasText: "BESA – Inter University Award" })
			.click();
